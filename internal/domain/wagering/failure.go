...
// FAILED (§7). Codes are part of the external contract: they travel in the
...
const (
	...
	// another player: one code for both denies an enumeration oracle (§2).
...
const (
	// Distinct from FailureReversalExceedsBalance by §7: a bet refused for
	...
	// processed movement to reverse (§7).
	FailureReferenceNotProcessed FailureCode = "REFERENCE_NOT_PROCESSED"
	// A reversal of this kind already succeeded against the reference (§7).
	FailureReferenceAlreadyReversed FailureCode = "REFERENCE_ALREADY_REVERSED"
	// Permanent infrastructure failure recorded for audit (§6.3 FAILED).
...
// Correctable reports whether a corrected resubmission could succeed (§7).
